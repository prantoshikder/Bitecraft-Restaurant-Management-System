    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="relative hidden overflow-hidden bg-ink lg:block">
        <div className="absolute inset-0">
          <img src={IMG.steakDark} alt="" className="size-full object-cover opacity-30" />
        ...
        <div className="relative flex h-full flex-col justify-between p-12">
          <Logo />
          <div>
            <span className="font-script text-3xl text-brand-light">Welcome back</span>
            ...
            <p className="mt-4 max-w-md text-sm text-white/50">
              Orders, reservations, menu, staff and analytics — everything you need to run BiteCraft,
              in one beautiful dashboard.
            </p>
          </div>
          <p className="text-xs text-white/40">© {new Date().getFullYear()} BiteCraft Restaurant.</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center bg-cream px-6 py-12">
        ...
        >
          ...
          <h2 className="mt-8 text-2xl font-extrabold text-ink lg:mt-0">Sign in to Dashboard</h2>
          <p className="mt-1 text-sm text-muted">Enter your credentials to access the admin panel.</p>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn btn-primary w-full">
              {loading ? "Signing in..." : (<>Sign In <FiArrowRight className="size-4" /></>)}
            </button>
          </form>

          <div className="mt-8 rounded-2xl border border-ink/8 bg-white p-5">
            <p className="text-[12px] font-semibold uppercase tracking-wider text-muted">Demo Accounts</p>
