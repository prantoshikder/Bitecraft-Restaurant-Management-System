      ...
      <article className="bg-cream py-16 lg:py-20">
        <div className="container-x max-w-3xl">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-5 text-[13px] text-muted">
            ...
            <span className="inline-flex items-center gap-1.5">
              <FiUser className="size-4 text-brand" /> {post.author}
            </span>
            <span className="inline-flex items-center gap-1.5">
              <FiTag className="size-4 text-brand" /> {post.category}
            </span>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {related.map((r) => (
              <Link
                key={r.id}
                href={`/blog/${r.slug}`}
                className="group overflow-hidden rounded-2xl border border-ink/8 bg-white transition-all hover:-translate-y-1.5 hover:shadow-(--shadow-card)"
              >
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-1 text-sm font-bold text-ink group-hover:text-brand">
                    {r.title}
                  </h3>
                </div>
              </Link>
