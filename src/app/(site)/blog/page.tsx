          ...
          <Stagger className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <StaggerItem key={post.id}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group flex h-full flex-col overflow-hidden rounded-2xl border border-ink/8 bg-white transition-all hover:-translate-y-1.5 hover:shadow-(--shadow-card)"
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <div className="flex items-center gap-4 text-[11px] text-muted">
                      ...
                      <span className="inline-flex items-center gap-1">
                        <FiUser className="size-3" /> {post.author}
                      </span>
                    </div>
